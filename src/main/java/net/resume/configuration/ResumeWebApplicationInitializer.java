...
import net.resume.component.impl.ApplicationListener;
import net.resume.component.impl.DebugFilter;
import net.resume.component.impl.ErrorHandler;

//главный обьект конфигурации создает контейнер где хранятся все бины из контекста и на стартапе записывает его в сервлет контекст
public class ResumeWebApplicationInitializer implements WebApplicationInitializer {
	...
	//создает контекст веб-приложения и загружает его в сервлеь контекст
	private WebApplicationContext createWebApplicationContext(ServletContext container) {
		AnnotationConfigWebApplicationContext ctx = new AnnotationConfigWebApplicationContext();
		ctx.scan("net.resume.configuration");
		ctx.setServletContext(container);
		ctx.refresh();
		return ctx;
	}
	//регистрирует все фильтры приложения
	...
	//исключения для статических ресурсов при использовании шаблонизатора
